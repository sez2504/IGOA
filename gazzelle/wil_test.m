...
clc
%pool = parpool(8);
SearchAgents_no = 300; % Number of search solutions
...
GOAM_matrix=zeros(30,50);
GOA_matrix=zeros(30,50);

for i = 4:30 % Loop over each function from F1 to F30
    % if i==2
    %     continue
    % end
    ...
    for run = 1:50
        [aoa_score, ~ , ~] = GOA2(SearchAgents_no, Max_iteration, lb, ub , dim , fobj);
        [maoa_score, ~ , ~] = GOA17(SearchAgents_no, Max_iteration, lb, ub , dim , fobj);
        aoa_scores(run) = aoa_score;
        maoa_scores(run) = maoa_score;
        GOAM_matrix(i,run)=maoa_score;
        GOA_matrix(i,run)=aoa_score;
    end
