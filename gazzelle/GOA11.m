...
function [Top_gazelle_fit,Top_gazelle_pos,Convergence_curve]=GOA11(SearchAgents_no,Max_iter,lb,ub,dim,fobj)
     ...
     for k=1:size(predator,1)
        ...
        if temp<distance_reqd
          ...
          for j=1:size(gazelle,2)
              ...
              % stepPredator=25+(35-25).*rand();
              stepPredator=RL(i,j)*(Elite(i,j)-RL(i,j)*predator(k,j));
              predator(k,j)=predator(k,j)+CF*sPredator*R*stepPredator;
                    if ubsize(1,2)>1
                        if predator(k,j)>ub(1,j)
                            predator(k,j)=ub(1,j);
                        end
