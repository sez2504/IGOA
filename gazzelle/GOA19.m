...
function [Top_gazelle_fit,Top_gazelle_pos,Convergence_curve]=GOA19(SearchAgents_no,Max_iter,lb,ub,dim,fobj)
...
% end
 ...
 for i=1:size(gazelle,1)
     ...
     for j=1:size(indices,2)
        indice=indices(1,j);
        avg_distance=avg_distance+norm(gazelle(indice,:)-gazelle(i,:));
     end
 end
avg_distance=avg_distance/(SearchAgents_no*(Total-SearchAgents_no));
        ...
        if ispresent==true
            for j=1:size(gazelle,2)
                for k=1:size(gazelle,1)
                    r=rand();
                    if(norm(gazelle(k,:)-gazelle(i,:))<avg_distance*0.89)
                        ispresent=false;
                        for l=1:size(indices,2)
                            if indices(1,l)==k
                                ispresent=true;
