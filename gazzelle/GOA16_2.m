...
function [Top_gazelle_fit,Top_gazelle_pos,Convergence_curve]=GOA16_2(SearchAgents_no,Max_iter,lb,ub,dim,fobj)
                ...
                for k=1:size(gazelle,1)
                    ...
                    if(r>0.5)
                        if(~ismember(k,indices))
                            stepsize=sign(RL(i,j)*(gazelle(k,j)-RL(i,j)*gazelle(i,j)));
                        ...
                        if(~ismember(k,indices))
                            stepsize=sign(RB(i,j)*(gazelle(k,j)-RB(i,j)*gazelle(i,j)));
                            gazelle(i,j)=gazelle(i,j)+sPredator*R*stepsize;  
                        else
