...
function [Top_gazelle_fit,Top_gazelle_pos,Convergence_curve]=GOA16_3(SearchAgents_no,Max_iter,lb,ub,dim,fobj)
...
% end
 ...
 gazelle_done=zeros(SearchAgents_no);
     ...
     for i = 1:size(gazelle,1)
        ...
        if ispresent==true
% Logical indexing for Predator selection (vectorized)
              is_predator = ~ismember(1:Total, indices);

% Pre-calculate some intermediate values (vectorized)
               %RL_mu = RL' * [reshape(mu,[],1); ones(dim-1,1)];
               RB = RB;  % Assuming RB doesn't change within the loop

                for j = 1:size(gazelle,2)
                    RL_j = RL(:,j);
                    gazelle_j = gazelle(:,j);
    
    % Step size calculations based on predator/gazelle selection (vectorized)
                     curr_RL=(RL .* (RL .* gazelle - Elite));
                     curr_RL=curr_RL(:,j);
                     curr_RB=(RB .* (RB .* gazelle - Elite));
                     curr_RB=curr_RB(:,j);
                     stepsize_RL = curr_RL .* is_predator';
                     stepsize_RB = curr_RB .* ~is_predator';

    
    % Update gazelle positions (vectorized)
                    gazelle(:,j) = Elite(:,j) + Spredator*R.*stepsize_RL.*mu + sPredator*R.*stepsize_RB.*mu;
                end
    ...
    % Step size calculations based on predator/gazelle selection (vectorized)
               ...
               gazelle(:,j) = gazelle(:,j) + S.*R.*stepsize_RL.*mu.*r + s*R.*stepsize_RB.*~r;
            end
        end
     end      
