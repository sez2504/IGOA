...
function [Top_gazelle_fit,Top_gazelle_pos,Convergence_curve]=GOA12(SearchAgents_no,Max_iter,lb,ub,dim,fobj)
...
while Iter<Max_iter    
 ...
 for i=1:size(gazelle,1)
     ...
     for k=1:size(predator,1)
        ...
        R=rand();
        if temp<distance_reqd*1.1
            for j=1:size(gazelle,2)
                stepsize(i,j)=RL(i,j)*(predator(k,j)-RL(i,j)*gazelle(i,j));
...
for i=1:size(predator,1)
     ...
     if predator_done(1,i)==1
         ...
         for j=1:size(predator,2)
            ...
            predator(i,j)=predator(i,j)+Spredator*R*stepPredator;
         end
     end
end
