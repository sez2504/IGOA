...
%         e-Mail: jiaheminglucky99@1
% 
% 26.com;rao12138@163.com                                                           
%                                   
%                                                                                                                                                   
...
function [best_fun,best_position,cuve_f,global_Cov]  =COA_eng(N,T,lb,ub,dim,fobj,VioFactor)
%% Define Parameters
cuve_f=zeros(1,T); 
X=initialization_coa(N,dim,ub,lb); %Initialize population
...
for i=1:N
    ...
    [fitness_f(i),g,h]=fobj(X(i,:));
    v=sum(VioFactor.*max(0,[g h]));
    fitness_f(i)=fitness_f(i)+v;
...
while(t<=T)
    ...
    for i = 1:N
        if temp>30
            ...
            v=sum(VioFactor.*max(0,[g h]));
            fitn=fitn+v;
            P = 3*rand*fitness_f(i)/fitn; %Eq.(4)
    ...
    [ global_fitness,g,h]=fobj(global_position);
    v=sum(VioFactor.*max(0,[g h]));
    global_fitness= global_fitness+v;
  
 
    for i =1:N
         %% Obtain the optimal solution for the updated population

        [new_fitness,g,h]=fobj(Xnew(i,:));
        v=sum(VioFactor.*max(0,[g h]));
        new_fitness=new_fitness+v;
