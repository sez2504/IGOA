...
function [Top_gazelle_fit,Top_gazelle_pos,Convergence_curve]=GOA7(SearchAgents_no,Max_iter,lb,ub,dim,fobj)
...
while Iter<Max_iter    
 ...
 for i=1:size(gazelle,1) %har row
      ...
      %predator=predator+mupredator*Spredator*R*10;    
       ...
       %       mu=1;
       % end
        temp=norm(predator - gazelle(i, :));
          %------------------ Exploitation ------------------- 
       if norm(predator - gazelle(i, :)) > (30e+02)
          count=count+1;
          ...
          %str=func2str(fobj);
          if ubsize(1,2)>1
            if predator(1,j)>ub(1,j)
                predator(1,j)=ub(1,j);
            end
            if predator(1,j)<lb(1,j)
               predator(1,j)=lb(1,j);
            end
          else
             if predator(1,j)>ub
